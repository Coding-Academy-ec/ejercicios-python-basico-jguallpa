...
def par_o_impar(numero):
    if numero  %2 == 0: # Se verifica si el número es divisible por 2
        return "par" # Se devuelve "par"
    else:
        return "impar" # Se devuelve "impar"

if __name__ == "__main__":
    num = int(input("Ingrese un número: "))  #  ingrese un número
...
def area_circulo(radio):
    ...
    return area  # se devuelve el area calculada

if __name__ == "__main__":
    radio = float(input("Ingrese el radio del círculo: "))  # Se solicita al usuario que ingrese el radio del círculo
    print("Area del circulo :", area_circulo(radio) )    # Se imprime el área calculada del círculo


# 6 Define una función que reciba dos números como argumentos y devuelva su suma.
def suma(a, b):
    x = a + b # Se devuelve la suma de los dos números recibidos como argumentos
    return x
